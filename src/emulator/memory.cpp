...
namespace emulator
{
    ...
    {
        return (static_cast<register_16_t>(read_byte(address + 1)) << 0x8u) | static_cast<register_16_t>(read_byte(address));
    }
    ...
    RomBank::RomBank(const register_16_t size, const register_16_t offset) : log{Logger::get_logger("memory_bank")},
                                                                             m_buffer{},
                                                                             m_offset{offset}
    {
    ...
    CartridgeBank::CartridgeBank(const std::string &file_name, register_16_t offset) : RomBank(0x0000, offset)
    {
        load_rom(file_name);
    }
    ...
    void CartridgeBank::load_rom(const std::string &rom_file)
    {
        ...
        if (!file.good())
        {
            // TODO: bad order dependency
            if (rom_file == emulator::boot_room)
            {
                throw std::runtime_error(fmt::format("file {} does not exist", rom_file));
            }
            else
            {
                m_buffer.resize(0);
    ...
    RamBank::RamBank(register_16_t size, register_16_t offset) : RomBank(size, offset)
    {
    }
    ...
    VideoRamBank::VideoRamBank(register_16_t size, register_16_t offset) : RamBank(size, offset)
    {
    }

    DualMappedRamBank::DualMappedRamBank(register_16_t size, register_16_t offset,
                                         register_16_t second_size, register_16_t second_offset) : RamBank(size, offset),
    ...
    Memory::Memory(const std::string &boot_rom, const std::string &game_rom) : m_boot_rom{boot_rom, 0x0000},
    ...
    IoRamBank::IoRamBank(Memory &mem, register_16_t size, register_16_t offset) : RamBank(size, offset),
                                                                                  m_mem{mem}
    {
    }
